        ...
        foreach (var page in pages)
        {
            ...
            foreach (var parsed in parsedVouchers)
            {
                bool isValid = parsed.Confidence >= 80 &&
                               parsed.FuelType != FuelType.Unknown &&
                               parsed.Liters > 0 &&
                ...
                if (!isValid)
                {
                    var errMsg = $"Voucher failed validation. Confidence: {parsed.Confidence}. FuelType: {parsed.FuelType}, Liters: {parsed.Liters}, Expiry: {parsed.ExpirationDate}, Number: {parsed.VoucherNumber}, QR Payload: {parsed.QrPayload}.";
